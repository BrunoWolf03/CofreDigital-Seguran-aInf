...
public class Base32 {
    ...
    public String toString(byte[] b) {
        ...
        for (int i = 0; i < (b.length + 4) / 5; i++) {
            ...
            // lower 3 bits from first byte, upper 2 bits from second
            t[1] = ((s[0] & 0x07) << 2) | ((s[1] >> 6) & 0x03);

            // bits 5-1 from second
            t[2] = (s[1] >> 1) & 0x1F;

            // lower 1 bit from second, upper 4 from third
            t[3] = ((s[1] & 0x01) << 4) | ((s[2] >> 4) & 0x0F);

            // lower 4 from third, upper 1 from fourth
            t[4] = ((s[2] & 0x0F) << 1) | ((s[3] >> 7) & 0x01);

            // bits 6-2 from fourth
            t[5] = (s[3] >> 2) & 0x1F;

            // lower 2 from fourth, upper 3 from fifth
     ...
     * @return An array containing the binary data, or null if invalid
     */
    public byte[] fromString(String str) {
        ...
        for (int i = 0; i < in.length / 8; i++) {
            ...
            // all 5 bits of first, high 3 of second
            t[0] = (s[0] << 3) | (s[1] >> 2);

            // lower 2 of second, all 5 of third, high 1 of fourth
            t[1] = ((s[1] & 0x03) << 6) | (s[2] << 1) | (s[3] >> 4);

            // lower 4 of fourth, high 4 of fifth
            t[2] = ((s[3] & 0x0F) << 4) | ((s[4] >> 1) & 0x0F);

            // lower 1 of fifth, all 5 of sixth, high 2 of seventh
            t[3] = (s[4] << 7) | (s[5] << 2) | (s[6] >> 3);

            // lower 3 of seventh, all of eighth
            t[4] = ((s[6] & 0x07) << 5) | s[7];
